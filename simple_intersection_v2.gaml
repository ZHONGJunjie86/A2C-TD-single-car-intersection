  ...
  species bus skills: [advanced_driving,RSkill] { 
	...
	action road_weight{
		if(m=0){
		 ...
		 if(int(distance_all/100)!=0){
		     time_target <- int(distance_all/100)*15+ rnd(3);
		 }
		 else{
		 	time_target <- rnd(15) + 2;
		 }
	...
	action reward_calculate{
    	if(time_pass != 0){
		  if(real_speed<=target_speed){
			  reward <- 0.005 - 0.001*(target_speed - real_speed); //0.001 * (real_speed/target_speed);//+ 0.01*(time_pass > time_target ? -1 : 1)*done; //
		  }
		  else{
			  reward <-  0.005 - 0.001*(real_speed - target_speed) ;//0.001 -((real_speed - target_speed)*0.03)/target_speed;//-((real_speed - target_speed)*0.003)/target_speed + 0.01*(time_pass > time_target ? -1 : 1)*done; //0.001 
		  }
	    }
	    else{reward<-0.0;}
	}
    action Python_A3C {
    	 ...
    	 save [real_speed, target_speed, elapsed_time_ratio, distance_left,reward,done,over] //time_pass
    	                                to: "D:/Software/GamaWorkspace/Python/GAMA_intersection_data_1.csv"  type: "csv" header: false; 
    	 save [real_speed, target_speed, elapsed_time_ratio, distance_left,reward,done,over] 
	...
	//目的地（終着バスターミナル）についた時の処理  && 最初の時   location distance_to final_target
	 reflex time_to_restart when:(distance_left)<=1 or first_time = 1 {  //a_flag_checked_by_light = 0 and checked = false
         ...
         write "target_time "+ time_target+"target_speed "+target_speed+" distance_all "+distance_all;
	} 
